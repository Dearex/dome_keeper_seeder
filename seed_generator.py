...
class seed():
    ...
    def lowest_hit_path(self, start, end, drillstrenght):
        hit_map = {vector: self.tile_hits(*vector, drillstrenght) for vector in self.hardness}
        path_hit_map = {}

        vectors = sorted(self.hardness, key=lambda vector:self.tile_dist(*vector, *end))
        path_hit_map[vectors[0]] = hit_map[vectors[0]]
        for vector in vectors[1:]:
            path_hit_map[vector] = hit_map[vector] + min(9999, *self.neighbour_tiles(path_hit_map, *vector))
        
        print(path_hit_map[start])
